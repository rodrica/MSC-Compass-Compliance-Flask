...
class Events(MethodView):
    ...
    def post(self, data: Dict[str, Any], dry_run: bool, current_user: AuthInfo, claims: ClaimSet):
        ...
        event = Event(**data)
        event.claims_by_audience = set_claims_default_tenant(data, current_user.tenant_id)
...
class EventById(MethodView):
    ...
    def get_event(self, current_user: AuthInfo, claims: ClaimSet, event_id: str):
        event = Event.get(current_user, event_id, claims=claims, raise_if_not_found=True)
    ...
    def put(self, data: Dict[str, Any], dry_run: bool, event_id: str, current_user: AuthInfo, claims: ClaimSet):
        ...
        event = self.get_event(current_user, claims.filter_by_action('read'), event_id)
        ...
        # no need to rollback on dry-run, flask-sqlalchemy does this for us.
        event.save(current_user, claims=claims.filter_by_action('update'), commit=not dry_run)
    ...
    def delete(self, dry_run: bool, event_id: str, current_user: AuthInfo, claims: ClaimSet):
        ...
        # no need to rollback on dry-run, flask-sqlalchemy does this for us.
        event.delete(current_user, claims=claims.filter_by_action('delete'), commit=not dry_run)
