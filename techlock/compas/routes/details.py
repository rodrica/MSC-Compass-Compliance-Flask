...
class Details(MethodView):
    ...
    def post(self, data: Dict[str, Any], dry_run: bool, current_user: AuthInfo, claims: ClaimSet):
        ...
        detail = Detail(**data)
        detail.claims_by_audience = set_claims_default_tenant(data, current_user.tenant_id)
...
class DetailById(MethodView):
    ...
    def get_detail(self, current_user: AuthInfo, claims: ClaimSet, detail_id: str):
        detail = Detail.get(current_user, detail_id, claims=claims, raise_if_not_found=True)
    ...
    def put(self, data: Dict[str, Any], dry_run: bool, detail_id: str, current_user: AuthInfo, claims: ClaimSet):
        ...
        detail = self.get_detail(current_user, claims.filter_by_action('read'), detail_id)
        ...
        # no need to rollback on dry-run, flask-sqlalchemy does this for us.
        detail.save(current_user, claims=claims.filter_by_action('update'), commit=not dry_run)
    ...
    def delete(self, dry_run: bool, detail_id: str, current_user: AuthInfo, claims: ClaimSet):
        ...
        # no need to rollback on dry-run, flask-sqlalchemy does this for us.
        detail.delete(current_user, claims=claims.filter_by_action('delete'), commit=not dry_run)
